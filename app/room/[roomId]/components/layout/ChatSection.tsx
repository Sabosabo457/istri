...
export function ChatSection({ combinedMessages, currentUserId, inputText, setInputText, onSendMessage, onFileSelect, onPreviewImage, scrollRef, fileInputRef }: ChatProps) {
  return (
    <aside className="w-full md:w-[420px] h-[50vh] md:h-full flex flex-col bg-white border-t-4 md:border-t-0 md:border-l-4 border-orange-50 shrink-0">
          ...
          return (
            <div key={m.id || i} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
              ...
              <div className={`max-w-[85%] px-4 py-2 rounded-[1.2rem] shadow-sm text-sm font-bold ${isMe ? "bg-orange-500 text-white rounded-tr-none" : "bg-white border-2 border-orange-100 text-orange-950 rounded-tl-none"}`}>
                {m.imageUrl ? <img src={m.imageUrl} onClick={() => onPreviewImage(m.imageUrl!)} className="rounded-xl" /> : <p className="whitespace-pre-wrap break-all leading-tight">{m.text}</p>}
              </div>
            </div>
          );
        })}
      ...
      <form onSubmit={onSendMessage} className="p-3 bg-white border-t-2 border-orange-50 shrink-0">
        <div className="flex items-center gap-1.5 bg-orange-50 p-1.5 rounded-[1.5rem] border-2 border-orange-100">
          <input 
          ...
          <button type="submit" className="w-9 h-9 bg-orange-500 text-white rounded-full flex items-center justify-center shadow-md shrink-0">🚀</button>
        </div>
      </form>
    </aside>
  );
