...
export default function LoginPage() {
  ...
  useEffect(() => {
    setMounted(true);
    // LINE からのアクセスの場合は、外部ブラウザでの起動を試みる
  ...
  return (
    <main className="h-screen bg-orange-50 text-orange-950 flex flex-col items-center justify-center p-8 relative overflow-hidden font-pop select-none">
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col items-center text-center">
        <div className="mb-10">
           <BrandHeader size="lg" align="center" />
        </div>

        <div className="w-full space-y-5">
           <button 
             onClick={handleGoogleLogin}
             className="w-full bg-white border-4 border-orange-100 hover:bg-orange-50 py-4 px-6 rounded-[1.5rem] shadow-[0_6px_0_#ffedd5] active:translate-y-1 transition-all flex items-center justify-center gap-4 group"
           >
              <svg className="w-6 h-6 group-hover:scale-110 transition-transform" viewBox="0 0 24 24"><path fill="#EA4335" d="M24 12.27c0-.85-.07-1.53-.22-2.27H12v4.51h6.61c-.35 1.83-1.42 3.39-2.9 4.34v3.58h4.74c2.81-2.58 4.45-6.39 4.45-10.16z"/><path fill="#34A853" d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-4.74-3.58c-1.25.84-2.81 1.34-4.54 1.34-3.58 0-6.61-2.42-7.69-5.69H.45v3.62C2.45 20.73 6.89 24 12 24z"/><path fill="#FBBC05" d="M4.31 13.16c-.28-.84-.44-1.74-.44-2.67s.16-1.83.44-2.67V4.2H.45a11.99 11.99 0 000 11.64l3.86-2.68z"/><path fill="#4285F4" d="M12 4.75c1.77 0 3.35.61 4.61 1.8l3.41-3.41C17.96 1.15 15.25 0 12 0 6.89 0 2.45 3.27.45 8.16l3.86 2.68c1.08-3.27 4.11-5.69 7.69-5.69z"/></svg>
              <span className="text-xl font-black text-orange-950">Googleログイン</span>
           </button>

           <div className="flex items-center gap-4">
              <div className="flex-1 h-0.5 bg-orange-100 rounded-full" />
              <span className="text-orange-200 font-black text-[10px] uppercase tracking-widest">OR</span>
              <div className="flex-1 h-0.5 bg-orange-100 rounded-full" />
           </div>

           <button 
             onClick={handleGuestLogin}
             className="w-full bg-orange-500 hover:bg-orange-400 py-4 px-6 rounded-[1.5rem] shadow-[0_6px_0_#c2410c] active:translate-y-1 transition-all group flex flex-col items-center justify-center gap-0.5"
           >
              <div className="text-xl font-black text-white group-hover:scale-105 transition-transform">ゲストではじめる</div>
              <div className="text-orange-200 text-[9px] font-black uppercase tracking-tighter">No Account Required</div>
           </button>
